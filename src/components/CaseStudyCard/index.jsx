    ...
    <div className="bg-white rounded-3xl lg:rounded-[48px] p-2 sm:p-4 h-full flex flex-col w-full hover:shadow-lg transition-shadow duration-500 group">
      <div className="flex justify-center bg-[#eeeff1] rounded-2xl lg:rounded-[40px] p-4 h-48 md:h-56 lg:h-64 xl:h-95">
        <Image
          src={data?.image}
          width={600}
          className="rounded-2xl lg:rounded-3xl lg:max-w-[540px] group-hover:scale-105 transition-transform duration-500 object-contain h-full w-auto"
      ...
      <div className="px-3 mt-6 xl:mt-3 flex-grow flex flex-col">
        ...
        <p className="text-text-secondary text-sm sm:text-base lg:text-lg max-w-lg  mt-3 mb-3 flex-grow">
          {data?.description}
        </p>
      </div>
      <div className="flex justify-between items-end p-2">
        <div className="flex gap-2 lg:gap-3 flex-wrap">
          {data?.tags.map((item, index) => (
            <div
              key={index}
              className="bg-[#C4C7CE33] text-text-primary p-1 rounded-full text-xs sm:text-sm lg:text-base inline-block px-3 lg:px-5 py-2"
            >
              {item}
            </div>
          ))}
        </div>
        <div className="relative w-8 h-8 md:w-10 md:h-10 lg:w-12 lg:h-12 border-text-secondary border border-solid rounded-full flex items-center justify-center flex-shrink-0 group-hover:border-primary group-hover:text-primary">
          {/* <Image
            src={arrowGreySvg}
            alt="Arrow icon"
            width={24}
          ...
          <svg
            ...
            className="text-foreground  lg:w-6.5 lg:h-6.5 transition-transform duration-500 ease-in-out group-hover:text-primary group-hover:-rotate-45"
          >
            <path
