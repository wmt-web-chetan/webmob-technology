...
export default function IntelligenceReturn({
  ...
  mainDescription =
    "Partnering with us isn't an expense; it's an investment in your competitive advantage. Our AI-augmented process delivers a measurable return across the three most critical areas of your business.",
  cards = [
    {
      icon: bulb,
    ...
    <section className="px-wrapper section-padding-y mx-auto">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-start">
        <div className="space-y-6">
          <div className="space-y-4 max-w-3xl">
            ...
            <p className="h2-description max-w-2xl  text-center sm:text-start ">{mainDescription}</p>
          </div>
        </div>
        ...
        <div className="p-6 bg-white rounded-3xl shadow-sm border border-text-disabled hover:border-blue-600 transition-colors duration-300 flex flex-col h-full">
          <div className="space-y-4 flex flex-col flex-grow">
            ...
            <div className="space-y-2 mt-4">
              ...
              <p className="text-gray-600 leading-relaxed">{cards[0].description}</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-12">
        {cards.slice(1).map((card, index) => (
          <div
            key={index}
            className="p-6 bg-white rounded-3xl shadow-sm border border-text-disabled hover:border-blue-600 transition-colors duration-300 flex flex-col h-full"
          >
            <div className="space-y-4 flex flex-col flex-grow">
              <div
                className="w-16 h-16 flex items-center justify-center rounded-[16px] bg-white"
